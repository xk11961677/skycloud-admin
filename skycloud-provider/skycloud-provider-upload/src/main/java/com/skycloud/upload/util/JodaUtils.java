...
public class JodaUtils {
	 ...
	 * @return
	 */
	public static Integer dateSub(String startDate, String endDate) {
		try {
			DateTime sDate = new DateTime(JodaUtils.parseDate(startDate, JodaUtils.DATE_FORMAT));
			DateTime eDate = new DateTime(JodaUtils.parseDate(endDate, JodaUtils.DATE_FORMAT));
	...
	public static String dateStrPlusDays(String text, int days) {
		return JodaUtils.parseDateTime(text, JodaUtils.DATE_FORMAT).plusDays(days).toString(JodaUtils.DATE_FORMAT);
	}
	...
	private static Integer getLastRepayDate(int ahead) {
		...
		if (DayOfMonth >= 1 && DayOfMonth <= 28) {// 1号到28号，还款日期加一个月
			...
			lastRepayDate = Integer
					.valueOf(JodaUtils.now().plusMonths(2).dayOfMonth().withMinimumValue().plusDays(ahead).toString(JodaUtils.DATE_FORMAT2));
		}
